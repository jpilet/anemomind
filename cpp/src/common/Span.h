...
public:
	...
	bool initialized() const {return _minv <= _maxv;}

	Span(double value) : _minv(value), _maxv(value) {}
	Span(double a, double b);


	double getMinv() {return _minv;}
	double getMaxv() {return _maxv;}

	void extend(const Span &other);
	void extend(double value);

	bool contains(double value) {return _minv <= value && value < _maxv;}
