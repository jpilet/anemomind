...
exports.create = function(req, res) {
  ...
  if (!req.body.boat) {
    return res.status(400).send('Missing boat field');
  }

  if (!stringIsObjectId(req.body.boat)) {
    return res.status(400).send('boat id is invalid');
  }
...
exports.postPhoto = multer({storage: multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    var filename = normalizePhotoFilename(file.originalname);
    var uuidRegexp = 
      /^[A-F0-9]{8}-[A-F0-9]{4}-4[A-F0-9]{3}-[89AB][A-F0-9]{3}-[A-F0-9]{12}\.jpg$/;

    if (filename.match(uuidRegexp)) {
      console.log('photoHandler: accepting file: ' + filename);
      cb(null, filename);
    } else {
...
function normalizePhotoFilename(filename) {
  return ('' + filename).toUpperCase().replace('.JPG','.jpg');
}

exports.photoUploadPath = photoUploadPath;
exports.normalizePhotoFilename = normalizePhotoFilename;
