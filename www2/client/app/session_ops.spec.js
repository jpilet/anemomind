...
describe('Edit ops', function() {
  ...
  it('Delete an entire session', function() {
    ...
    expect(tree.startTime).toBe(sessions[0].startTime);
    expect(tree.endTime).toBe(sessions[sessions.length-1].endTime);

    var leafCount = SessionOps.reduceSessionTreeLeaves(countFun, 0, tree);

    expect(sessions.length).toBe(leafCount);
    ...
    expect(totalDuration - totalDuration2).toBeCloseTo(durationToDelete);
    
    var leafCount2 = SessionOps.reduceSessionTreeLeaves(countFun, 0, tree2);
    expect(leafCount2 + 1).toBe(leafCount);
  });
  ...
  it('Delete from an empty list', function() {
    ...
    expect(renderedSessions instanceof Array).toBeTruthy();
    expect(renderedSessions.length).toBe(0);
  });
  ...
  it('Delete a session in the middle, making two sessions', function() {
    ...
    expect(leafCount2).toBe(sessions.length + 1);
    ...
    expect(sessions2[index]._id).toBeFalsy();
    expect(sessions2[index+1]._id).toBeFalsy();

    // But the other ones should have...
    expect(sessions2[index-1]._id).toBeTruthy();
    expect(sessions2[index+2]._id).toBeTruthy();

    var dif0 = (totalDur - totalDur2);
    var dif1 = SessionOps.sessionDurationSeconds(sessionToDelete) 
        - marginSeconds - marginSeconds;
    expect(dif0).toBeCloseTo(dif1);
  });
});
