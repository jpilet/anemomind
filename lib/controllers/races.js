...
exports.raceDetail = function(req, res) {
  ...
  Upload.findById(raceId, function (err, race) {
    ...
    fs.readFile(race.file, 'utf8', function (err, data) {
      ...
      var formattedData = {};
      formattedData.coords = JSON.parse(data);
      formattedData.origin = minAxis(formattedData.coords);
      var ref = new GeoRef(formattedData.origin.x, formattedData.origin.y, 0);

      for (var i = formattedData.coords.length-1; i >= 0; i--) {
        var projected = ref.project(formattedData.coords[i].lat_rad, formattedData.coords[i].lon_rad);
        formattedData.coords[i].xInMeters = projected.x;
        formattedData.coords[i].yInMeters = projected.y;
      }
...
function minAxis(array) {
  ...
  var lowestLon = Number.POSITIVE_INFINITY;
  var tmpLat, tmpLon;
  for (var i=array.length-1; i>=0; i--) {
      tmpLon = array[i].lon_rad;
      tmpLat = array[i].lat_rad;
      if (tmpLat < lowestLat) lowestLat = tmpLat;
      if (tmpLon < lowestLon) lowestLon = tmpLon;
  }
  return {x: lowestLat, y: lowestLon};
}
