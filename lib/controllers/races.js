  ...
  Upload.findById(raceId, function (err, race) {
    ...
    fs.readFile(race.file, 'utf8', function (err, data) {
      ...
      for (var i = formattedData.coords.length-1; i >= 0; i--) {
        var projected = ref.project(formattedData.coords[i]['lat-rad'], formattedData.coords[i]['lon-rad']);
...
function minAxis(array) {
  ...
  for (var i=array.length-1; i>=0; i--) {
    minAxis.x = Math.min(minAxis.x, array[i]['lat-rad']);
    minAxis.y = Math.min(minAxis.y, array[i]['lon-rad']);
  }
  return minAxis;
}
