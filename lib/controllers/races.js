...
exports.raceDetail = function(req, res) {
  ...
  Upload.findById(raceId, function (err, race) {
    ...
    fs.readFile(race.file, 'utf8', function (err, data) {
      ...
      var min = minAxis(data);
      var ref = new GeoRef(min.x, min.y, 0);

      for (var i = data.length-1; i >= 0; i--) {
        var projected = ref.project(data[i].lat_rad, data[i].lon_rad);
        data[i].xInMeters = projected.x;
        data[i].yInMeters = projected.y;
      }
      res.send(data);
    });
  });
