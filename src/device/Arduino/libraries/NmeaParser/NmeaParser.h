...
class AccAngle {
  public:
    ...
    void set(Word deg, Word min, Word mc) {
        deg_ = deg; min_ = min; mc_ = mc;
    }

    Word deg() const { return deg_; }
    Word min() const { return min_; }
    Word mc() const { return mc_; }

    AccAngle &operator = (double a) { set(a); return *this; }
...
class NmeaParser {
  public:
    ...
    DWord cumulativeWaterDistance() const { return cwd_; }
    DWord waterDistance() const { return wd_; }
    ...
    short gpsBearing() const { return gpsBearing_; } // degrees
    char hour() const { return hour_; }
    char min() const { return min_; }
    char sec() const { return sec_; }
    char day() const { return day_; }
    char month() const { return month_; }
    char year() const { return year_; }

    // Wind data
    short awa() const { return awa_; } // aparent wind angle [degrees]
    short twa() const { return twa_; } // true wind angle [degrees]
    Word aws() const { return aws_; } // apparent wind speed [1/256 knots]
    Word tws() const { return tws_; } // true wind speed [1/256 knots]
    Word magHdg() const { return magHdg_; } // magnetic heading [degrees]
    Word watSpeed() const { return watSpeed_; } // water speed [1/256 knots]
    DWord cwd() const { return cwd_; } // cumulative water distance [nautical miles]
    DWord wd() const { return wd_; } // water distance since reset [1/10 nautical miles]

    DWord numBytes() const { return numBytes_; }
    Word numErr() const { return numErr_; }
    Word numSentences() const { return numSentences_; }
