...
TEST(PolarHistogramTest, Undefined) {
 ...
 TEST(PolarHistogramTest, BasicTest) {
  ...
  EXPECT_EQ(map.angleToBinIndex(Angle<double>::degrees(1)), 0);
  EXPECT_EQ(map.angleToBinIndex(Angle<double>::degrees(121)), 1);
  EXPECT_EQ(map.angleToBinIndex(Angle<double>::degrees(241)), 2);
  EXPECT_EQ(map.angleToBinIndex(Angle<double>::degrees(361)), 0);
...
TEST(PolarHistogramTest, CustomReference) {
  ...
  EXPECT_EQ(map.angleToBinIndex(ref + marg), 1);
  EXPECT_EQ(map.angleToBinIndex(ref - marg), 0);
}

TEST(PolarHistogramTest, Negative) {
  PolarHistogramMap map(4);
  EXPECT_EQ(map.angleToBinIndex(Angle<double>::degrees(-1)), 3);
}
