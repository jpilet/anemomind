...
TEST(GemanMcClureTest, Modelfitting) {
  ...
  LevmarState state(arma::mat(X.ptr(), X.size(), 1, false, true));
  Arrayd Xopt = state.minimize(settings, fitnessRobust);
