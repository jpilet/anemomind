...
void LevmarState::step(const LevmarSettings &settings, Function &fun) {
  ...
  if (maxAbsElement(JtF) < settings.e1 || norm2(_Fscratch.n_elem, _Fscratch.memptr()) <= settings.e3) {
    SCOPEDMESSAGE(INFO, "Stop because optimum reached.");
    _stop = true;
  } else {
    assert(_mu > 0);
    ...
    while (!(_stop || rho > 0)) {
      ...
      if (arma::norm(dX, 2) < settings.e2*normX) {
        SCOPEDMESSAGE(INFO, "Stop because step size is minimum.");
        _stop = true;
      } else {
        Xnew = _X + dX;
        ...
        if (rho > 0 && // Improvement over previous estimate
          ...
          SCOPEDMESSAGE(INFO, "Improvement with this step size.");
          _X = Xnew;
          ...
          SCOPEDMESSAGE(INFO, "Increase damping.");
          _mu = _mu*_v;
          assert(!std::isnan(_mu));
          _v = 2*_v;
          assert(!std::isinf(_v));
